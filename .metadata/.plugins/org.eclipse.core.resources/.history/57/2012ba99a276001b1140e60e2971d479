...
$(window).scroll(function() {
	/*버그 수정 (근사치 계산)*/
    var checkNum = ($(window).scrollTop()) - ($(document).height() - $(window).height());
...
function make_feed_box(image){
    var feed_box = `<div class="photo u-default-box">`;
	...
	if(image.heart == true){
		feed_box += `<i onclick="onFeedLoad(${image.user.id})" id="${image.user.id}" class="fa fa-heart heart heart-clicked"></i>`;
	}else{
		feed_box += `<i onclick="onFeedLoad(${image.user.id})" id="${image.user.id}" class="fa fa-heart-o heart"></i>`;
	}
		
	feed_box += `</span> <span class="photo__action">`;
	feed_box += `<i class="fa fa-comment-o"></i></span></div>`;
	
	feed_box += `<span class="photo__likes" id="photo_likes_count_${image.user.id}">${image.likeCount}</span><span class="photo__likes"> likes</span><div class="photo_caption">`;
	feed_box += `<span class="photo__username">${image.user.username} </span>`;
...
async function load_feed_box(){
	...
	var response = await fetch(`/feed/scroll?page=${page}`);
	var images = await response.json();
	
	console.log(images);
	
	images.forEach(function(image){
		var feed_box = make_feed_box(image);
